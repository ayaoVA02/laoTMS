...
const recentActivityItems = [
  ...
  { id: "a3", text: "Attraction 'Night Market' approved by staff", time: "6 hours ago", icon: CheckCircle, color: "text-emerald-500" },
  { id: "a4", text: "New user registration: Mike T.", time: "8 hours ago", icon: Users, color: "text-teal-500" },
  { id: "a5", text: "Promotion created: Early Bird Special", time: "1 day ago", icon: Bell, color: "text-rose-500" },
];
...
export default function DashboardPage() {
  ...
  const [editingAttraction, setEditingAttraction] = useState<(typeof attractions)[0] | null>(null);
  const [newAttraction, setNewAttraction] = useState({
    name: "", description: "", category: "", price: "", location: "", socialShare: false,
  });
  ...
  const [socialShareStates, setSocialShareStates] = useState<Record<string, boolean>>(
    () => attractions.reduce((acc, a) => ({ ...acc, [a.id]: a.socialShare }), {} as Record<string, boolean>)
  );
  ...
  const handleCreateAttraction = () => {
    if (!newAttraction.name || !newAttraction.category || !newAttraction.location) return;
    const created = {
      id: String(localAttractions.length + 1),
      name: newAttraction.name, description: newAttraction.description,
      shortDescription: newAttraction.description.slice(0, 60),
      category: newAttraction.category, location: newAttraction.location,
      coordinates: [17.9757, 102.6331] as [number, number], images: [],
      rating: 0, reviewCount: 0, price: Number(newAttraction.price) || 0,
      openTime: "09:00", closeTime: "17:00", facilities: [],
      status: "pending" as const, featured: false,
      createdAt: new Date().toISOString().split("T")[0],
      entrepreneurId: "3", entrepreneurName: user?.name || "Entrepreneur",
      socialShare: newAttraction.socialShare,
    };
    setLocalAttractions((prev) => [...prev, created]);
    setSocialShareStates((prev) => ({ ...prev, [created.id]: created.socialShare }));
    setNewAttraction({ name: "", description: "", category: "", price: "", location: "", socialShare: false });
    setCreateDialogOpen(false);
  };
  const handleEditAttraction = () => {
    if (!editingAttraction) return;
    setLocalAttractions((prev) => prev.map((a) => (a.id === editingAttraction.id ? editingAttraction : a)));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-teal-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-teal-950/20 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-6">Sign in to access your personalized dashboard with role-based features.</p>
  ...
  // ─── STAT CARD ──────────────────────────────────────────────────
  const StatCard = ({ title, value, change, up, icon: Icon, accent }: {
    title: string; value: string | number; change: string; up: boolean;
      ...
      <Card className="relative overflow-hidden border-0 shadow-md hover:shadow-lg transition-shadow">
        ...
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="flex items-center gap-1 mt-2 sm:mt-3">
            ...
            <span className={`text-xs sm:text-sm font-medium ${up ? "text-emerald-500" : "text-red-500"}`}>{change}</span>
  ...
  const renderAdminDashboard = () => (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard title={t("dashboard.totalAttractions")} value={localAttractions.length} change="+12%" up icon={Building2} accent="from-teal-500 to-emerald-600" />
        <StatCard title={t("dashboard.totalUsers")} value={191} change="+8%" up icon={Users} accent="from-sky-500 to-blue-600" />
        <StatCard title={t("dashboard.totalReviews")} value={reviews.length} change="+23%" up icon={Star} accent="from-amber-500 to-orange-600" />
            ...
            <CardContent>
              <div className="flex items-end gap-1.5 sm:gap-2 h-36 sm:h-48">
                {chartData.map((d, i) => {
                  ...
                  return (
                    <motion.div key={d.month} className="flex-1 flex flex-col items-center gap-1" initial={{ height: 0 }} animate={{ height: "auto" }} transition={{ delay: i * 0.06, duration: 0.5 }}>
                      <span className="text-[10px] sm:text-xs text-muted-foreground font-medium">{d.value}</span>
                      <motion.div className="w-full rounded-t-md bg-gradient-to-t from-teal-500 to-emerald-400" initial={{ height: 0 }} animate={{ height: `${heightPercent}%` }} transition={{ delay: 0.2 + i * 0.06, duration: 0.6, ease: "easeOut" }} style={{ minHeight: 4 }} />
              ...
              {roleDistribution.map((r) => (
                <div key={r.role} className="space-y-2">
                  ...
                  <div className="relative h-2 w-full overflow-hidden rounded-full bg-secondary">
                    <div className="h-full bg-gradient-to-r from-teal-500 to-emerald-500 transition-all" style={{ width: `${(r.count / totalRoleUsers) * 100}%` }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            {pendingAttractions.length === 0 ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                {pendingAttractions.map((attraction) => (
                  <motion.div key={attraction.id} layout className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 rounded-xl border bg-card hover:shadow-sm transition-shadow gap-3 sm:gap-0">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h4 className="font-semibold text-sm truncate">{attraction.name}</h4>
                        <Badge variant="outline" className={statusBadgeMap.pending.className}>{t("attractions.pending")}</Badge>
                      </div>
                      <p className="text-xs text-muted-foreground truncate">{attraction.location} - {attraction.category}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">Submitted by {attraction.entrepreneurName}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm" onClick={() => handleApprove(attraction.id)}>
                        <CheckCircle className="w-3.5 h-3.5 sm:mr-1" />
                        <span className="hidden sm:inline">Approve</span>
                      </Button>
                      <Button size="sm" variant="destructive" className="text-xs sm:text-sm" onClick={() => handleReject(attraction.id)}>
        ...
        <Card className="border-0 shadow-md">
          ...
          <CardContent>
            <div className="space-y-2 sm:space-y-3">
              {localAttractions.slice().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5).map((a) => (
                <div key={a.id} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] sm:text-xs text-muted-foreground">{a.entrepreneurName} - {a.createdAt}</p>
                  </div>
                  <Badge variant="outline" className={`${statusBadgeMap[a.status]?.className || ""} text-[10px] sm:text-xs shrink-0`}>
  ...
  const renderEntrepreneurDashboard = () => (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4 sm:space-y-6">
      ...
      {(() => {
        ...
        const oldAttractions = myAttractions.filter((a) => new Date(a.createdAt) <= sixMonthsAgo);
            ...
            <Card className="border-0 shadow-md border-l-4 border-l-amber-500">
              <CardHeader className="pb-2 sm:pb-3">
                <CardTitle className="text-base sm:text-lg font-semibold flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="bg-amber-500/15 text-amber-600 border-amber-500/25 text-[10px] ml-auto">{oldAttractions.length}</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs text-muted-foreground mb-3">These attractions may need updates or fresh promotions to stay relevant.</p>
                <div className="space-y-2">
                  {oldAttractions.map((a) => (
                    <div key={a.id} className="flex items-center gap-3 p-2.5 sm:p-3 rounded-xl bg-amber-500/5 border border-amber-500/10">
                      <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-amber-500/20 to-orange-500/20 flex items-center justify-center shrink-0 overflow-hidden">
                        {a.images[0] ? <img src={a.images[0]} alt={a.name} className="w-full h-full object-cover" /> : <Building2 className="w-4 h-4 text-amber-500" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs sm:text-sm font-medium truncate">{a.name}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-muted-foreground">Created {a.createdAt}</span>
                          <Badge variant="outline" className={`${statusBadgeMap[a.status]?.className || ""} text-[9px]`}>{statusBadgeMap[a.status]?.label || a.status}</Badge>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-teal-600" onClick={() => { setEditingAttraction({ ...a }); setEditDialogOpen(true); }}>
        ...
        <Card className="border-0 shadow-md">
          <CardHeader className="pb-2 sm:pb-4">
            <div className="flex items-center justify-between gap-2">
              ...
              <Button size="sm" className="bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white shadow-sm shrink-0" onClick={() => setCreateDialogOpen(true)}>
                <Plus className="w-4 h-4 sm:mr-1.5" />
                <span className="hidden sm:inline">{t("dashboard.createAttraction")}</span>
              </Button>
            </div>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {myAttractions.map((attraction) => (
                <div key={attraction.id} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl border bg-card hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{attraction.location} - {attraction.category}</p>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 pl-15 sm:pl-0 border-t sm:border-t-0 pt-3 sm:pt-0">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      <Switch checked={socialShareStates[attraction.id] ?? attraction.socialShare} onCheckedChange={() => toggleSocialShare(attraction.id)} className="scale-90 sm:scale-100" />
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-teal-600" onClick={() => { setEditingAttraction({ ...attraction }); setEditDialogOpen(true); }}>
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-red-600" onClick={() => handleDeleteAttraction(attraction.id)}>
        ...
        <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input id="new-name" placeholder="Attraction name" value={newAttraction.name} onChange={(e) => setNewAttraction((prev) => ({ ...prev, name: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="new-desc" className="text-sm">Description</Label>
              <Textarea id="new-desc" placeholder="Describe your attraction" value={newAttraction.description} onChange={(e) => setNewAttraction((prev) => ({ ...prev, description: e.target.value }))} className="min-h-[80px]" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-sm">Category</Label>
                <Select value={newAttraction.category} onValueChange={(val) => setNewAttraction((prev) => ({ ...prev, category: val }))}>
              ...
              <div className="space-y-2">
                <Label htmlFor="new-price" className="text-sm">Price (LAK)</Label>
                <Input id="new-price" type="number" placeholder="0" value={newAttraction.price} onChange={(e) => setNewAttraction((prev) => ({ ...prev, price: e.target.value }))} />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="new-location" className="text-sm">Location</Label>
              <Input id="new-location" placeholder="City or region" value={newAttraction.location} onChange={(e) => setNewAttraction((prev) => ({ ...prev, location: e.target.value }))} />
            </div>
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-center w-full h-20 sm:h-24 rounded-lg border-2 border-dashed border-muted-foreground/25 hover:border-teal-500/50 transition-colors cursor-pointer">
                <div className="text-center">
                  ...
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Click to upload images</p>
                </div>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-medium">{t("dashboard.socialShare")}</p>
                  <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{t("dashboard.enableSocialShare")}</p>
                </div>
              </div>
              <Switch checked={newAttraction.socialShare} onCheckedChange={(checked) => setNewAttraction((prev) => ({ ...prev, socialShare: checked }))} />
            </div>
          </div>
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setCreateDialogOpen(false)} className="w-full sm:w-auto">{t("common.cancel")}</Button>
            <Button className="w-full sm:w-auto bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white" onClick={handleCreateAttraction}>{t("common.submit")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {editingAttraction && (
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                ...
                <Input id="edit-name" value={editingAttraction.name} onChange={(e) => setEditingAttraction((prev) => prev ? { ...prev, name: e.target.value } : prev)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-desc" className="text-sm">Description</Label>
                <Textarea id="edit-desc" value={editingAttraction.description} onChange={(e) => setEditingAttraction((prev) => prev ? { ...prev, description: e.target.value } : prev)} className="min-h-[80px]" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-sm">Category</Label>
                  <Select value={editingAttraction.category} onValueChange={(val) => setEditingAttraction((prev) => prev ? { ...prev, category: val } : prev)}>
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-price" className="text-sm">Price (LAK)</Label>
                  <Input id="edit-price" type="number" value={editingAttraction.price} onChange={(e) => setEditingAttraction((prev) => prev ? { ...prev, price: Number(e.target.value) } : prev)} />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-location" className="text-sm">Location</Label>
                <Input id="edit-location" value={editingAttraction.location} onChange={(e) => setEditingAttraction((prev) => prev ? { ...prev, location: e.target.value } : prev)} />
              </div>
            </div>
          )}
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setEditDialogOpen(false)} className="w-full sm:w-auto">{t("common.cancel")}</Button>
            <Button className="w-full sm:w-auto bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white" onClick={handleEditAttraction}>{t("common.save")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </motion.div>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {[
                ...
                <Button key={action.label} variant="outline" className="h-auto py-3 sm:py-4 flex flex-col items-center gap-1.5 sm:gap-2 border-teal-500/20 hover:bg-teal-500/10 hover:border-teal-500/40 transition-all" asChild>
                  <a href={action.href}>
          ...
          <CardContent>
            <div className="space-y-3 sm:space-y-4">
              {(reviews || []).slice(0, 4).map((review) => {
                const attraction = (localAttractions || []).find((a) => a.id === review.attractionId);
                return (
                  <div key={review.id} className="p-3 sm:p-4 rounded-xl border bg-card">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed line-clamp-2">{review.comment}</p>
  ...
  const renderContent = () => {
    switch (role) {
      ...
      case "ENTREPRENEUR": return renderEntrepreneurDashboard();
      case "TOURIST":
      default: return renderTouristDashboard();
    }
  };
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1 truncate">
                  {roleGreeting[role] || "Dashboard"} - Welcome back, {user?.name || "Guest"}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
