...
import { Users, Search, Shield, Mail, Calendar, MoreHorizontal, UserPlus, X } from "lucide-react";
...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ name: "", email: "", role: "TOURIST" });
  ...
  const handleAddUser = () => {
    ...
    setNewUser({ name: "", email: "", role: "TOURIST" });
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(roleCounts).map(([role, count]) => (
            <motion.div key={role} variants={itemVariants}>
              <Card className="border-0 shadow-md text-center cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setFilterRole(filterRole === role ? "all" : role)}>
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  ...
                  <Button size="sm" className="bg-gradient-to-r from-teal-500 to-emerald-600 text-white shrink-0" onClick={() => setAddDialogOpen(true)}>
                      ...
                      <tr key={u.id} className="border-b border-border/30 hover:bg-muted/30 transition-colors">
                        <td className="py-3 pr-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-white text-sm font-bold shrink-0">{u.name.charAt(0)}</div>
                            <div className="min-w-0"><p className="text-sm font-medium truncate">{u.name}</p><p className="text-xs text-muted-foreground truncate">{u.email}</p></div>
                          </div>
                        </td>
                        <td className="py-3 pr-4"><Badge variant="outline" className={roleBadgeColors[u.role]}>{t(`roles.${u.role.toLowerCase()}`)}</Badge></td>
                        <td className="py-3 pr-4 text-sm text-muted-foreground">{u.joined}</td>
                        <td className="py-3 pr-4"><Badge variant="outline" className={u.status === "active" ? "bg-emerald-500/15 text-emerald-600 border-emerald-500/25" : "bg-slate-500/15 text-slate-500 border-slate-500/25"}>{u.status}</Badge></td>
                        <td className="py-3 text-right"><Button variant="ghost" size="icon" className="h-8 w-8"><MoreHorizontal className="w-4 h-4" /></Button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="sm:hidden space-y-2">
                {filtered.map((u) => (
                  <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl border bg-card">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-white text-sm font-bold shrink-0">{u.name.charAt(0)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{u.name}</p>
                      <p className="text-xs text-muted-foreground truncate">{u.email}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className={`${roleBadgeColors[u.role]} text-[10px]`}>{t(`roles.${u.role.toLowerCase()}`)}</Badge>
                        <Badge variant="outline" className={`${u.status === "active" ? "bg-emerald-500/15 text-emerald-600 border-emerald-500/25" : "bg-slate-500/15 text-slate-500 border-slate-500/25"} text-[10px]`}>{u.status}</Badge>
                      </div>
                    </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input id="user-name" placeholder="Enter full name" value={newUser.name} onChange={(e) => setNewUser((p) => ({ ...p, name: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="user-email" className="text-sm">Email</Label>
              <Input id="user-email" type="email" placeholder="user@example.com" value={newUser.email} onChange={(e) => setNewUser((p) => ({ ...p, email: e.target.value }))} />
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button className="bg-gradient-to-r from-teal-500 to-emerald-600 text-white" onClick={handleAddUser} disabled={!newUser.name || !newUser.email}>
              <UserPlus className="w-4 h-4 mr-1.5" />
              Add User
            </Button>
          </DialogFooter>
