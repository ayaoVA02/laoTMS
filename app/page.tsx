...
export default function Home() {
  ...
  // Dynamic Popular Destinations based on attraction counts
  const popularDestinations = useMemo(() => {
    ...
    attractions.forEach(attr => {
      const p = provinces.find(prov =>
        ...
        attr.location?.toLowerCase().includes(prov.province_en.toLowerCase().replace(' province', '').replace(' prefecture', ''))
      );
      if (p) {
        counts[p.province_en] = (counts[p.province_en] || 0) + 1;
      }
    });
